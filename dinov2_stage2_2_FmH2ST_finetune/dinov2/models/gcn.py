...
class GATConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        self_loop_attr[:, -1] = 1  # 最后一维表示 self-loop
        self_loop_attr = self_loop_attr.to(edge_attr.device).to(edge_attr.dtype)
        # 合并边特征
        edge_attr = torch.cat((edge_attr, self_loop_attr), dim=0)

        # 边特征编码
        edge_embeddings = self.edge_encoder(edge_attr) #.view(-1, self.heads, self.emb_dim) 
        # 离散输入处理
        if self.input_layer:
            x = self.input_node_embeddings(x.to(torch.int64).view(-1,))## -->[1280,128]
        # 节点特征线性变换 + 多头 reshape
        x = self.weight_linear(x).view(-1, self.heads, self.emb_dim)#-->[1280,2,128]
...
#     """
#     def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin",edge_dim=9):
...
#             elif gnn_type == "gat":
#                 self.gnns.append(GATConv(emb_dim, input_layer = input_layer,edge_dim=edge_dim))
...
class GNNChunk(torch.nn.ModuleList):
    """
    A ModuleList wrapper for chunking multiple GNN layers into one logical unit.
    It behaves as a container (ModuleList) so FSDP can wrap instances of this class.
    We do NOT implement forward here because the outer GNN.forward will iterate
    through each layer and still collect per-layer outputs (h_list) for JK pooling.
    """
    pass
...
class GNN(torch.nn.Module):
    def __init__(
        ...
        # build raw layer list
        raw_layers = []
        self.norms = torch.nn.ModuleList()
        for layer in range(num_layer):
            input_layer = False
            if gnn_type == "gin":
                raw_layers.append(GINConv(emb_dim, aggr="add", input_layer=input_layer))
            elif gnn_type == "gcn":
        ...
        # If block_chunks > 1, group raw_layers into chunks; else keep as flat ModuleList
        if block_chunks and block_chunks > 1:
            self.chunked = True
            chunksize = max(1, num_layer // block_chunks)
            chunked = []
            for i in range(0, num_layer, chunksize):
                sub = raw_layers[i : i + chunksize]
                chunked.append(GNNChunk(sub))
            # self.gnns holds ModuleList of either individual layers (if not chunked) or GNNChunk instances
            self.gnns = torch.nn.ModuleList(chunked)
        else:
            self.chunked = False
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        We'll iterate chunk-by-chunk, and inside iterate each layer to append per-layer outputs.
        """
        h_list = [x]
        if not self.chunked:
            # same as original
            ...
            layer_idx = 0
            # self.gnns is ModuleList of GNNChunk (each is iterable of layers)
        ...
        if self.JK == "last":
            ...
            tmp = [h.unsqueeze(0) for h in h_list[1:]]
            node_representation = torch.sum(torch.cat(tmp, dim=0), dim=0)
        else:
            raise NotImplementedError(f"JK mode {self.JK} not implemented in this snippet")
...
class GNN_graphpred(torch.nn.Module):
    ...
    JK-net: https://arxiv.org/abs/1806.03536
    """
    def __init__(self, num_layer, emb_dim, num_tasks, JK = "last", drop_ratio = 0, graph_pooling = "mean", gnn_type = "gin", block_chunks=1):
        ...
        self.gnn = GNN(num_layer, emb_dim, JK, drop_ratio, gnn_type = gnn_type, block_chunks=block_chunks)
