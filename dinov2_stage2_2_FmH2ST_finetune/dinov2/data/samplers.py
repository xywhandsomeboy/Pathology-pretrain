...
class WeightedShardedInfiniteSampler(Sampler):
    ...
    def __init__(
        ...
        *,
        ...
        if weights is not None:
            ...
            self._weights = weights / weights.sum()  # 归一化
        else:
            self._weights = None
    ...
    def __iter__(self):
        ...
        if self._weights is not None:
            iterator = self._weighted_iterator()
        elif self._shuffle:
            iterator = self._shuffled_iterator()
        else:
            iterator = self._iterator()
    ...
    def _weighted_iterator(self):
        """
        加权采样迭代器
        使用torch.multinomial进行加权采样
        """
        generator = torch.Generator()
        generator.manual_seed(self._seed)
        ...
        while True:
            ...
            # 转换为numpy并应用分布式切片
            dtype = _get_numpy_dtype(num_samples)
            indices_np = indices.numpy().astype(dtype)
            
            # 应用分布式切片（只返回当前进程负责的样本）
            iterable = itertools.islice(indices_np, self._start, None, self._step)
            yield from iterable
            self._iter_count += 1
