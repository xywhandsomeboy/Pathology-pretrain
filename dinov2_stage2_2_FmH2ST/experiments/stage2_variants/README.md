...
| 2 | `distance_only` | 边只由坐标距离 KNN 和最大距离阈值决定；图中不保存 `edge_attr`，GATv2 不读取边权 | 仅保留节点重建和图对比；删除边存在预测、边权回归和边权噪声 | 同一纯距离、无边权图 |
...
版本 2 也不再预测由坐标规则确定的边是否存在。它只通过节点重建和图对比学习
空间邻域内的上下文表示；图的建立始终由坐标 KNN 与最大距离阈值负责。
...
Boundary F1 和 HD95。版本 2 没有边存在预测头和边权回归头，因此其
`edge_existence` 与 `edge_weight` 指标固定为零，不能与版本 1/3 的对应损失横向比较。
