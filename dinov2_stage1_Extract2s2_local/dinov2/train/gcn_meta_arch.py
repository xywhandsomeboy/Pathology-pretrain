...
class GCNMetaArch(nn.Module):
    ...
    @torch.no_grad()
    def extract_local_patch_tokens(self, images):
        n_global_crops = 2
        assert n_global_crops == 2
        n_local_crops = self.cfg.crops.local_crops_number
        ...
        student_local_backbone_output_dict = self.student.backbone(
            local_crops, masks=None, is_training=False
        )
        local_patch_tokens = student_local_backbone_output_dict["x_norm_patchtokens"]
        # reshape to (n_local_crops, batch, n_tokens, dim)
        dim = local_patch_tokens.shape[-1]
        n_tokens = local_patch_tokens.shape[1]
        batch_size = local_crops.shape[0] // n_local_crops
        local_patch_tokens = local_patch_tokens.view(n_local_crops, batch_size, n_tokens, dim)
        return local_patch_tokens

    def forward_backward(self, images):
        if not self.training:
            return self.extract_local_patch_tokens(images)
