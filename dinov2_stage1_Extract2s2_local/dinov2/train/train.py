...
def do_test(cfg, model):
    ...
    dense_tile_size = 96
    dense_tile_stride = 96
    dense_tile_batch_size = 256
    ...
    def _compute_start_positions(length: int, tile: int, stride: int):
        if length <= tile:
            return [0]
    ...
    def _extract_dense_tiles(image_tensor: torch.Tensor, tile: int, stride: int):
        # image_tensor: (C,H,W)
        c, h, w = image_tensor.shape
        ys = _compute_start_positions(h, tile, stride)
        xs = _compute_start_positions(w, tile, stride)
        tiles = []
        coords = []
        for y in ys:
            for x in xs:
                tiles.append(image_tensor[:, y : y + tile, x : x + tile])
                coords.append((y, x))
    ...
    for idx, data in enumerate(data_loader):
        ...
        if images is None:
            raise RuntimeError("Expected 'images' in batch. Ensure DataAugmentationDINO(return_original=True) and collate returns images.")

        # process each 256x256 patch-image independently (save per patch-image)
        for b in range(images.shape[0]):
            image = images[b].to(torch.float).cuda(non_blocking=True)
            tiles, coords = _extract_dense_tiles(image, dense_tile_size, dense_tile_stride)

            feats_list = []
            for start in range(0, tiles.shape[0], dense_tile_batch_size):
                tile_batch = tiles[start : start + dense_tile_batch_size]
                out = model.student.backbone(tile_batch, masks=None, is_training=False)
                patch_tokens = out["x_norm_patchtokens"]  # (T, n_tokens, dim)
                tile_feats = patch_tokens.mean(dim=1)  # (T, dim)
                feats_list.append(tile_feats.detach().cpu())

            feats = torch.cat(feats_list, dim=0).numpy()
            coords_np = coords.numpy()

            base = os.path.basename(filenames[b])
            stem = os.path.splitext(base)[0]
            np.savez_compressed(os.path.join(output_dir, f"{stem}_tile96s96_feat.npz"), feats)
            np.savez_compressed(os.path.join(output_dir, f"{stem}_tile96s96_coord_yx.npz"), coords_np)
